...
func discloseProof(t *testing.T, cred Credential, indices []int) DisclosureProof {
	t.Helper()
...
func TestDisclosureIsBoundToTheIssuerKey(t *testing.T) {
	...
	dp := discloseProof(t, cred, []int{2})

	if !dp.Verify([]gabikeys.PublicKey{pk}) {
		t.Fatal("a disclosure did not verify under the key that issued it")
	}
	if dp.Verify([]gabikeys.PublicKey{otherPk}) {
...
func TestSeveralIssuerKeysAllWork(t *testing.T) {
	for i := 0; i < 2; i++ {
		...
		cred := issueCredential(t, sk, pk, attrs, 1)
		if !discloseProof(t, cred, []int{3}).Verify([]gabikeys.PublicKey{pk}) {
...
func TestCredentialIssuedWithKeysLoadedFromDisk(t *testing.T) {
	...
	if !discloseProof(t, cred, []int{4}).Verify([]gabikeys.PublicKey{loadedPk}) {
...
func TestVerifyRejectsAWrongNumberOfPublicKeys(t *testing.T) {
	attrs := randomAttributes(t, 8)
	sk, pk := newIssuerKey(t)
	dp := discloseProof(t, issueCredential(t, sk, pk, attrs, 1), []int{2})

	if dp.Verify(nil) {
		t.Error("verification succeeded with no public keys")
	}
	if dp.Verify([]gabikeys.PublicKey{pk, pk}) {
