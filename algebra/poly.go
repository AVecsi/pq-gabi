	...
	for layer < N {
		for offset := 0; offset < N-layer; offset += 2 * layer {
			...
			for j := offset; j < offset+layer; j++ {
				...
				cs[j] = bigCS.Mul(bigCS, bigINV2).Mod(bigCS, big.NewInt(Q)).Int64()

				cs[j+layer] = bigINV2.Mul(bigINV2, big.NewInt(int64(z))).Mul(bigINV2, big.NewInt(int64(t))).Mod(bigINV2, big.NewInt(Q)).Int64()
			}
		}
		layer *= 2
	}
